...
export default function VigilanteSidebar({ isOpen, onToggle }) {
  const location = useLocation();
  const { logout, user } = useAuth();

  const isActive = (path) => location.pathname.startsWith(`/vigilante${path}`);

  const menuItems = [
    { icon: "📊", label: "Dashboard", path: "/dashboard" },
    { icon: "📝", label: "Registrar Ingreso", path: "/registrar" },
  ];

  return (
    <>
      {isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 md:hidden z-40" onClick={onToggle} />
      )}
      <aside className={`${
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Panel Vigilante</p>
            </div>
          </div>
        </div>
